...
//  revealNextAttacker
  ...
  return (Rmagic(target, board.occupiedSquares) & (blackQueens | whiteQueens | blackRooks | whiteRooks))
  | (Bmagic(target, board.occupiedSquares) & (blackQueens | whiteQueens | blackBishops | whiteBishops))
  | (KNIGHT_ATTACKS[target] & (blackKnights | whiteKnights))
  | (BLACK_PAWN_ATTACKS[target] & (whitePawns))
  | (WHITE_PAWN_ATTACKS[target] & (blackPawns))
  | (KING_ATTACKS[target] & (blackKing | whiteKing));
}
