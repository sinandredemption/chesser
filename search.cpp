...
Move Board::think()
{
	...
	//  iterative deepening with aspirational windows.
	const int WindowSize = 40;
	int LargeWindowSize = LARGE_NUMBER;
	int alpha = -LARGE_NUMBER, beta = LARGE_NUMBER;
	/*for (currentdepth = 1; currentdepth <= searchDepth; currentdepth++)
	{*/
	for (currentdepth = 1; currentdepth <= searchDepth;)
	{
		//  clear the buffers:
		...
		//score = alphabetaLMR(0, currentdepth, alpha, beta); // Fails
		...
		// If the nodes fail low or high, my idea was to use
		// alpha/beta = score +- LargeWindowSize
		// But after reading SF code, I found something better.
		if (score <= alpha) {
			alpha = MAX(score - LargeWindowSize, -LARGE_NUMBER);
			LargeWindowSize /= 2;
			continue;
		} else if (score >= beta) {
			beta = MIN(score + LargeWindowSize, LARGE_NUMBER);
			LargeWindowSize /= 2;
			continue;
		}
		...
		if (currentdepth >= 9) {
			...
			alpha = MAX(score - WindowSize, -LARGE_NUMBER);
			beta = MIN(score + WindowSize, LARGE_NUMBER);
		}
...
int Board::alphabetapvs(int ply, int depth, int alpha, int beta)
{
	...
	if (LARGE_NUMBER - ply <= alpha)
		return alpha;
	else if (-LARGE_NUMBER + ply >= beta)
		return beta;
