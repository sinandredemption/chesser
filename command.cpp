...
noPonder:
		...
		// Added in Chesser 1.5
		if (XB_MODE && !strcmp(command, "bench")) 
		{ 
//			XB_COMPUTER_SIDE = XB_ANALYZE;			
//			continue; 
		}
		...
		{ 
			...
			std::cout << "Chesser> search depth " << board.searchDepth << std::endl;
			continue; 
		}
		...
		{ 
			...
			std::cout << "Chesser> stopfrac " << 100*STOPFRAC << std::endl;
			continue; 
		}
		...
		{ 
			...
			if (number < 1) number = 1;
			if (!XB_MODE) std::cout << "Chesser> search time " << number << " seconds" << std::endl;
			if (XB_MODE)
			{
				XB_CTIM = number * 10;
