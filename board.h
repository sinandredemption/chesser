...
struct Board
{
	...
	int alphabetaLMR(int ply, int depth, int alpha, int beta);
  ...
  int repetitionCount();
	...
	int SEE(Move &move);
	inline BitMap attacksTo(int target);
	inline BitMap revealNextAttacker(BitMap &attackers, BitMap &nonremoved, int &target, int &heading);
