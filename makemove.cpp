...
void makeMove(Move &move)
{
	...
	case 2: // white king:
		...
		if (move.isCastle())
		{
			if (move.isCastleOO())
			{
				const unsigned H1F1 = (1 << 7) | (1 << 5);
				assert(H1F1 == (BITSET[H1] | BITSET[F1]));
				/*board.whiteRooks         ^= BITSET[H1] | BITSET[F1];
				board.whitePieces        ^= BITSET[H1] | BITSET[F1];
				board.occupiedSquares    ^= BITSET[H1] | BITSET[F1];*/
				board.whiteRooks         ^= H1F1;
				board.whitePieces        ^= H1F1;
				board.occupiedSquares    ^= H1F1;
			...
			{
				const unsigned A1D1 = (1 << 0) | (1 << 3);
				assert(A1D1 == (BITSET[A1] | BITSET[D1]));
				/*board.whiteRooks         ^= BITSET[A1] | BITSET[D1];
				board.whitePieces          ^= BITSET[A1] | BITSET[D1];
				board.occupiedSquares    ^= BITSET[A1] | BITSET[D1];*/
				board.whiteRooks         ^= A1D1;
				board.whitePieces          ^= A1D1;
				board.occupiedSquares    ^= A1D1;
	...
	case 10: // black king:
		...
		if (move.isCastle())
		{
			if (move.isCastleOO())
			{
				const U64 H8F8 = (1ULL << 63) | (1ULL << 61);
				assert(H8F8 == (BITSET[H8] | BITSET[F8]));
				/*board.blackRooks         ^= BITSET[H8] | BITSET[F8];
				board.blackPieces          ^= BITSET[H8] | BITSET[F8];
				board.occupiedSquares    ^= BITSET[H8] | BITSET[F8];*/
				board.blackRooks         ^= H8F8;
				board.blackPieces          ^= H8F8;
				board.occupiedSquares    ^= H8F8;
			...
			{
				const U64 A8D8 = (1ULL << 56) | (1ULL << 59);
				assert(A8D8 == (BITSET[A8] | BITSET[D8]));
				/*board.blackRooks         ^= BITSET[A8] | BITSET[D8];
				board.blackPieces          ^= BITSET[A8] | BITSET[D8];
				board.occupiedSquares    ^= BITSET[A8] | BITSET[D8];*/
				board.blackRooks         ^= A8D8;
				board.blackPieces          ^= A8D8;
				board.occupiedSquares    ^= A8D8;
...
BOOLTYPE isOwnKingAttacked()
{
	...
	if (board.nextMove)
	{
		assert(board.bkingsq == INDEX64[(board.blackKing * DEBRUIJN64) >> 58]);
		return isSqAttacked(board.bkingsq, false);
	}
	else
	{
		assert(board.wkingsq == INDEX64[(board.whiteKing * DEBRUIJN64) >> 58]);
		//ksq = INDEX64[(board.blackKing * DEBRUIJN64) >> 58];
		return isSqAttacked(board.wkingsq);
	}
}
