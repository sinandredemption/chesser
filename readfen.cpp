...
BOOLTYPE readFen(char *filename, int number)
{
    ...
    // open the file for read and scan through until we find the number-th position:
       ...
       if (fp != NULL)
       {
              ...
              while (fscanf(fp, "%s", s) != EOF) 
              {
                     ...
                     if (!strcmp(s, "[FEN"))
                     {
                           ...
                           if (numberf == number)
                           {
                                  ...
                                  std::cout << std::endl << "Chesser> fen #" << numberf << " in " << filename << ":" << std::endl << std::endl;
              ...
              if (numberf < number)
              {
                     printf("Chesser> only %d fens present in %s, fen #%d not found\n",
       ...
       {
              printf("Chesser> error opening file: %s\n", filename);
              returnValue = false;
       }
       return returnValue;
}
       ...
       while ((counter < 64) && (fen[i] != '\0'))
       {
              ...
              //  other characters:
              {
                     switch (fen[i])
                     {
                           ...
                           case 'K':
														 board.wkingsq = BOARDINDEX[file][rank];
                           ...
                           case 'k':
														 board.bkingsq = BOARDINDEX[file][rank];
